...
# Split on EVERY dated entry header, then classify by task id.
# NOTE: do not put `[TB]-\d+` in the split pattern. Entries with no task id exist
# (e.g. `## 2026-08-06 — [Orchestrator, /explore]`); if the pattern requires an id,
# re.split does not cut there and that entry is silently concatenated onto the
# PREVIOUS task's file, misattributed to a task that never took it.
...
for section in task_sections:
    ...
    match = re.match(r'^## \d{4}-\d{2}-\d{2} — ([TB]-\d+)', section)
    if match:
        task_id = match.group(1)
        if task_id not in task_map:
            task_map[task_id] = []
        task_map[task_id].append(section)
    else:
        untagged.append(section)   # entries with no task id
...
if untagged:
    with open('context/decisions/general.md', 'w', encoding='utf-8') as f:
        f.write('\n\n---\n\n'.join(untagged) + '\n')
    print(f'Created context/decisions/general.md ({len(untagged)} untagged entries)')

print(f'\nTotal task files created: {len(task_map)}')
EOF
```

**Verifica la integridad por conteo, no por rango de IDs** — es la única comprobación que
detecta una entrada perdida o mal atribuida:

```bash
# los dos pares deben coincidir
grep -c '^## ' context/decisions.md
cat context/decisions/*.md | grep -c '^## '

grep -c '^Decided:' context/decisions.md
cat context/decisions/*.md | grep -c '^Decided:'
```
...
# Debe incluir:
# context/decisions/<un fichero por ID que APAREZCA en decisions.md>
#   — habrá huecos: no toda tarea deja decisiones registradas, y puede haber IDs
#     fuera del rango que esperes. No cuentes ficheros: usa el conteo de cabeceras
#     de arriba para verificar que no se ha perdido nada.
# context/decisions/general.md        (entradas sin task-id, si las hay)
# context/decisions/legacy-header.md  (preámbulo del fichero plano)
