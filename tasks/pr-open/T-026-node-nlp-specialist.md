...
**Verification:** `pytest --cov=src --cov-fail-under=70` — 933 passed (96.84% total coverage), 1
deselected (`test_resume_after_restart_does_not_rerun_completed_phase` — pre-existing failure on
`origin/main` before this task's changes, documented at `context/discoveries.md:236`, unrelated to
`nlp_specialist`/`_experiment_design.py`). `src/nodes/llm/nlp_specialist.py` — 100% coverage (26/26
lines). `ruff check .` / `ruff format --check .` / `mypy src/` — all pass with zero issues.
