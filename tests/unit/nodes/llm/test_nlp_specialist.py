"""Unit tests for src/nodes/llm/nlp_specialist.py.
...
VALID_DESIGN: dict[str, Any] = {
    ...
    "rationale": "Short free-text field favors a fast sparse-linear baseline.",
}
RESPONSE_CONTENT = json.dumps(VALID_DESIGN)
SOLUTION_PLAN = {"model_families": ["tfidf_linear"], "order": ["tfidf_linear"]}
FOLD_CONFIG = {
...
def mock_workspace_manager():
    ...
    own `__call__`, which writes the output) and `src.nodes.llm.nlp_specialist`
    (the node's own instance, built in `_build_messages` to read the upstream
    artifacts) — resolving to the same mock instance, since neither call site
    ...
    with (
        ...
        patch("src.nodes.llm.nlp_specialist.WorkspaceManager") as mock_wm_cls_node,
    ):
...
def test_config_and_prompt_load_for_real() -> None:
    config = load_agent_config("nlp_specialist")

    assert config.name == "nlp_specialist"
    ...
    prompt = PromptLoader().load("nlp_specialist", "v1")
    assert prompt.strip() != ""
    assert "# System prompt — nlp_specialist" in prompt
...
def test_call_writes_design_with_search_space_and_model_family(
    ...
    assert args[1]["model_family"] == "tfidf_linear"
...
def test_specialist_field_injected_not_taken_from_llm(
    ...
    mock_llm.invoke.return_value = AIMessage(content=_design(specialist="classical_ml_specialist"))
    _, workspace_instance = mock_workspace_manager
    node = NlpSpecialistNode()

    node(_build_state())

    assert _written_design(workspace_instance)["specialist"] == "nlp_specialist"
...
def test_build_messages_includes_plan_and_fold_summary(
    ...
    assert "tfidf_linear" in last_message.content
...
def test_json_in_json_fence_is_parsed(
    ...
    assert _written_design(workspace_instance)["model_family"] == "tfidf_linear"
...
def test_invalid_json_raises_value_error(
    ...
    with pytest.raises(ValueError, match="nlp_specialist"):
        node(_build_state())
...
@pytest.mark.parametrize(
    ...
    [
        ("TF-IDF", "tfidf_linear"),
        ("SBERT", "sentence_embeddings"),
        ("DistilBERT fine-tuning", "transformer_finetune"),
    ],
)
...
def test_ambiguous_model_family_raises(
    patched_llm_factory, patched_settings, mock_workspace_manager, mock_llm: MagicMock
) -> None:
    """Naming two of the three families is rejected outright rather than
    resolved by precedence."""
    mock_llm.invoke.return_value = AIMessage(
        content=_design(model_family="tfidf or sentence embeddings")
    )
    node = NlpSpecialistNode()
